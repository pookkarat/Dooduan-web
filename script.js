...
function setupDailyCardRefresh() {
  const btn = document.getElementById('dailyRefreshBtn');
  if (!btn) return;
  btn.addEventListener('click', () => renderDailyCard(true));
}

function renderDailyCard(forceRandom = false) {
  if (typeof TAROT_CARDS === 'undefined' || !Array.isArray(TAROT_CARDS) || TAROT_CARDS.length === 0) return;

  const img = document.getElementById('dailyCardImage');
  const name = document.getElementById('dailyCardName');
  const meaning = document.getElementById('dailyCardMeaning');
  if (!img || !name || !meaning) return;

  let index = 0;
  if (forceRandom) {
    index = Math.floor(Math.random() * TAROT_CARDS.length);
  } else {
    const now = new Date();
    const seed = Number(`${now.getFullYear()}${now.getMonth()+1}${now.getDate()}`);
    index = seed % TAROT_CARDS.length;
  }

  const card = TAROT_CARDS[index];
  img.src = card.image || '';
  img.alt = card.name_th || card.name_en || 'ไพ่ประจำวัน';
  name.textContent = (card.name_th || card.name_en || '').split(' / ')[0];
  meaning.textContent = card.meaning_general || 'วันนี้ไพ่ชวนให้ค่อย ๆ ใช้สติและฟังเสียงหัวใจของตัวเอง';
}


const ZODIAC_DATA = {
  aries: { label: 'เมษ', element: 'ไฟ', note: 'กล้าตัดสินใจ ตรงไปตรงมา และมีแรงผลักดันสูง' },
  taurus: { label: 'พฤษภ', element: 'ดิน', note: 'ให้ความสำคัญกับความมั่นคง ความชัดเจน และความสบายใจ' },
  gemini: { label: 'เมถุน', element: 'ลม', note: 'คิดไว สื่อสารเก่ง และต้องการข้อมูลก่อนตัดสินใจ' },
  cancer: { label: 'กรกฎ', element: 'น้ำ', note: 'อ่อนไหว ลึกซึ้ง และรับพลังของความสัมพันธ์ได้ดี' },
  leo: { label: 'สิงห์', element: 'ไฟ', note: 'มีเสน่ห์ ต้องการความชัดเจน และอยากได้รับการเห็นคุณค่า' },
  virgo: { label: 'กันย์', element: 'ดิน', note: 'ละเอียด รอบคอบ และมักคิดวิเคราะห์ก่อนเชื่อความรู้สึก' },
  libra: { label: 'ตุลย์', element: 'ลม', note: 'มองหาความสมดุล ความยุติธรรม และการสื่อสารที่นุ่มนวล' },
  scorpio: { label: 'พิจิก', element: 'น้ำ', note: 'รู้สึกลึก อ่านบรรยากาศเก่ง และจริงจังกับความผูกพัน' },
  sagittarius: { label: 'ธนู', element: 'ไฟ', note: 'รักอิสระ ต้องการคำตอบที่เปิดทางและไม่ถูกจำกัด' },
  capricorn: { label: 'มังกร', element: 'ดิน', note: 'จริงจังกับเป้าหมาย ความรับผิดชอบ และผลลัพธ์ระยะยาว' },
  aquarius: { label: 'กุมภ์', element: 'ลม', note: 'มองต่างมุม ชอบเหตุผล และต้องการพื้นที่ของตัวเอง' },
  pisces: { label: 'มีน', element: 'น้ำ', note: 'อินกับความรู้สึก มีสัญชาตญาณ และมักรับพลังรอบตัวได้ไว' }
};
...
function setupTopicChips() {
  document.querySelectorAll('.topic-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      selectedTopicChoice = btn.dataset.topic || 'auto';
    });
  });
}
...
function setupZodiacControls() {
  ...
  birth.addEventListener('change', () => {
    const zodiac = getZodiacFromDate(birth.value);
    if (zodiac) select.value = zodiac;
    updateZodiacHint();
  });

  select.addEventListener('change', updateZodiacHint);
  updateZodiacHint();
}
...
function updateZodiacHint() {
  const select = document.getElementById('zodiacSelect');
  const hint = document.getElementById('zodiacHint');
  if (!select || !hint) return;
  const data = ZODIAC_DATA[select.value];
  if (!data) {
    hint.textContent = 'ยังไม่ได้ระบุราศี';
    return;
  }
  hint.textContent = `พลังราศี${data.label} ธาตุ${data.element}: ${data.note}`;
}

function captureProfileInputs() {
  const birth = document.getElementById('birthInput');
  const select = document.getElementById('zodiacSelect');
  currentBirthDate = birth?.value || '';
  currentZodiac = select?.value || '';
}

function zodiacLine() {
  const data = ZODIAC_DATA[currentZodiac];
  if (!data) return '';
  return `พลังส่วนตัวจากราศี${data.label} ธาตุ${data.element} สะท้อนว่า ${data.note}`;
}
...
function buildAiLikeReading(cards) {
  ...
  let html = `
    <div class="card-detail open ai-reading-panel">
      ...
      ${zodiacLine() ? `<p><strong>พลังราศี:</strong> ${zodiacLine()}</p>` : ''}
...
function buildGeminiPayload() {
  ...
  return {
    ...
    zodiac: currentZodiac ? (ZODIAC_DATA[currentZodiac]?.label || currentZodiac) : '',
    zodiacElement: currentZodiac ? (ZODIAC_DATA[currentZodiac]?.element || '') : '',
    zodiacNote: currentZodiac ? (ZODIAC_DATA[currentZodiac]?.note || '') : '',
...
function selectMode(mode) {
  ...
  currentTopic    = selectedTopicChoice !== 'auto' ? selectedTopicChoice : detectTopic(q);
  selectedCards   = [];
  pickedIndices   = [];
...
function showResultScreen() {
  ...
  currentTopic = selectedTopicChoice !== 'auto' ? selectedTopicChoice : detectTopic(currentQuestion);
  document.getElementById('resultQuestion').textContent = `"${currentQuestion}"`;
  buildResultCards();
  saveReadingHistory('ai-like');
...
function setupHistoryControls() {
  ...
  if (refresh) refresh.addEventListener('click', renderHistoryList);
  if (clear) clear.addEventListener('click', () => {
    if (!confirm('ต้องการล้างประวัติคำทำนายทั้งหมดในเครื่องนี้ใช่ไหม?')) return;
    localStorage.removeItem('dooduanReadingHistory');
    renderHistoryList();
    showToast('✦ ล้างประวัติแล้วค่ะ');
  });
}

function getReadingHistory() {
  try {
    return JSON.parse(localStorage.getItem('dooduanReadingHistory') || '[]');
  } catch (_) {
    return [];
  }
}

function saveReadingHistory(source = 'ai-like') {
  if (!currentQuestion || selectedCards.length !== 3) return;
  ...
  const item = {
    ...
    question: currentQuestion,
    topic,
    topicLabel: TOPIC_LABELS[topic] || 'ภาพรวมชีวิต',
    zodiac: currentZodiac,
    zodiacLabel: currentZodiac ? (ZODIAC_DATA[currentZodiac]?.label || '') : '',
    source,
    cards: selectedCards.slice(0, 3).map((card, i) => ({
      position: POSITIONS[i]?.label || `ใบที่ ${i + 1}`,
      ...
      image: card.image || '',
      meaning: getCardMeaning(card, topic)
    })),
    aiLikeSummary: synthesizeAdvice(selectedCards, topic),
    geminiReading: currentGeminiReading || ''
  };

  const filtered = history.filter(h => h.question !== item.question || JSON.stringify(h.cards?.map(c => c.name_en)) !== JSON.stringify(item.cards.map(c => c.name_en)));
  filtered.unshift(item);
  localStorage.setItem('dooduanReadingHistory', JSON.stringify(filtered.slice(0, 12)));
  renderHistoryList();
}
...
function renderHistoryList() {
  ...
  const history = getReadingHistory();
  if (!history.length) {
    list.innerHTML = '<div class="history-empty">ยังไม่มีประวัติคำทำนาย</div>';
    return;
  }

  list.innerHTML = history.map(item => {
    const date = new Date(item.date);
    const dateText = Number.isNaN(date.getTime()) ? '' : date.toLocaleString('th-TH', { dateStyle: 'medium', timeStyle: 'short' });
    const zodiac = item.zodiacLabel ? `<span class="history-pill">ราศี${escapeHtml(item.zodiacLabel)}</span>` : '';
    const cards = (item.cards || []).map(c => `${escapeHtml(c.position)}: ${escapeHtml((c.name_th || c.name_en || '').split(' / ')[0])}`).join(' · ');
    const summary = item.geminiReading ? item.geminiReading.slice(0, 220) + (item.geminiReading.length > 220 ? '...' : '') : item.aiLikeSummary;
    return `
      <article class="history-item">
        ...
        <div class="history-meta">
          <span class="history-pill">${escapeHtml(item.topicLabel || 'ภาพรวมชีวิต')}</span>
          ${zodiac}
          <span class="history-pill">${item.source === 'gemini' ? 'AI วิเคราะห์แล้ว' : 'AI-like'}</span>
        </div>
...
function copyResult() {
  ...
  if (zodiacLine()) text += `พลังราศี: ${zodiacLine()}\n`;
  text += `\n`;
